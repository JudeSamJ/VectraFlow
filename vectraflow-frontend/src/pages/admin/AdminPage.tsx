...
export function AdminPage() {
  ...
  const tripCB = useMutation({
    mutationFn: (name: string) =>
      apiClient.post(`/admin/circuit-breakers/${name}/trip`),
    onSuccess: () =>
  ...
  const resetCB = useMutation({
    mutationFn: (name: string) =>
      apiClient.post(`/admin/circuit-breakers/${name}/reset`),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["admin-circuit-breakers"] }),
  });
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {cbs.map((cb: any) => (
              ...
              >
                ...
                <Badge variant={cbVariant[cb.state as CircuitBreakerState]}>
                  {cb.state}
                </Badge>
                <Button
                  variant="icon"
      ...
      <Card>
        ...
        {dlqLoading ? (
          ...
          <div style={{ textAlign: "center", padding: "32px 0" }}>
            ...
            >
              No failed documents — everything is healthy ✓
            </p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
